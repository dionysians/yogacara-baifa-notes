...
REPLACEMENTS = [
    ...
    # ===== D 区复合术语(优先匹配) =====
    (r"\bfunctional substrate ordering\b", "功能根基次第"),  # v5 修正
    ...
    (r"\bbottleneck\b", "瓶颈"),
    (r"\bthreshold\b", "阈值"),
    (r"\bsubstrate\b", "根基"),  # v5 修正:原"基质"读起来化学/物理感太强
    (r"\bdimension\b", "维度"),
    (r"\bmanifest\b", "显现"),
    (r"\bcapacity\b", "承载力"),
    (r"\bframing\b", "框定"),
    ...
    # ===== v5 补遗 - polishing pass (2026-05-17 残留词捕捉) =====
    (r"\bstrict temporal sequence\b", "严格时序"),
    (r"\bstrict temporal\b", "严格时序"),
    (r"\bdynamic balance\b", "动态平衡"),
    (r"\bmulti-angle\b", "多角度"),
    (r"\blow stress\b", "低压力"),
    (r"\bin-session\b", "座中"),
    (r"\bperturbation\b", "扰动"),
    (r"\bdefinition\b", "定义"),
    (r"\bbehavior\b", "行为"),
    (r"\bhypothesis\b", "假说"),
    (r"\bterritory\b", "领域"),
    ...
    (r"\btrajectory\b", "轨迹"),
    (r"\bcaveat\b", "警示"),
    (r"\binternally consistent\b", "内部一致"),
    (r"\bultimately redundant\b", "最终冗余"),
    (r"\bnet\b", "净"),
    (r"\bhit\b", "命中"),
    (r"\bread\b", "读出"),
    (r"\bagent\b", "代理"),
    ...
    (r"\bgrounding\b", "立基"),
    # 短语和句式
    (r"\b倾向性的 state\b", "倾向性状态"),
    (r"\b自心 state\b", "自心状态"),
    (r"\b身心 feeling\b", "身心感受"),
    (r"\b身心 passive\b", "身心被动"),
    (r"\b身心 neutral\b", "身心中性"),
    (r"\b身心 strength\b", "身心强度"),
    (r"\bactor status\b", "行为者地位"),
    (r"\btrait-style\b", "特质式"),
]
